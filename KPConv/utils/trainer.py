...
class ModelTrainer:
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        for epoch in range(config.max_epoch):
            ...
            self.step = 0
            for counter, batch in enumerate(training_loader):
                ...
                # filenamesave = "/home/reza/PHD/Fall22/Olivier/KPConv-PyTorch/files/" + str(counter) + ".ply"
                # write_ply(filenamesave, [batch.points[0].numpy(), batch.labels.numpy().astype('int32')], field_names)
            ...
            # Validation
            net.eval()
            self.curr_miou=self.validation(net, val_loader, config)
            if self.curr_miou>self.best_miou:
                print("NEW RECORD on",self.epoch,"with miou of",self.curr_miou)
                print("************************")
                print()
                self.best_miou=self.curr_miou
                checkpoint_path = join(checkpoint_directory, 'best.tar')
                torch.save(save_dict, checkpoint_path)
            net.train()
    ...
    def validation(self, net, val_loader, config: Config):
        ...
        elif config.dataset_task == 'agg_segmentation':
            val_miou = self.agg_segmentation_validation(net, val_loader, config)
    ...
    def agg_segmentation_validation(self, net, val_loader, config, debug=True):
        """
        Validation method for slam segmentation models
        """
        ...
        for i, batch in enumerate(val_loader):
            ...
            for b_i, length in enumerate(lengths):
                ...
                if exists(filepath):
                    ...
                    frame_preds = np.zeros(frame_labels.shape[0], dtype=np.uint8)
                frame_preds[proj_mask] = preds.astype(np.uint8)
                np.save(filepath, frame_preds)
                ...
                # if f_ind % 20 == 0:
                #     seq_path = join(val_loader.dataset.path, 'sequences', val_loader.dataset.sequences[s_ind])
                #     velo_file = join(seq_path, 'velodyne', val_loader.dataset.frames[f_ind] + '.bin')
                #     frame_points = np.fromfile(velo_file, dtype=np.float32)
                #     frame_points = frame_points.reshape((-1, 4))
                #     write_ply(filepath[:-4] + '_pots.ply',
                #               [frame_points[:, :3], frame_labels, frame_preds],
                ...
                frame_C = fast_confusion(frame_labels,
                                         frame_preds.astype(np.int32),
                                         val_loader.dataset.label_values)
                val_loader.dataset.val_confs[f_ind][:, :] = frame_C
        ...
        mIoU = 100 * np.mean(IoUs)
        print('{:s} : subpart mIoU = {:.1f} %'.format(config.dataset, mIoU))
        val_mIoU = 100 * np.mean(val_IoUs)
        print('{:s} :     val mIoU = {:.1f} %'.format(config.dataset, val_mIoU))
