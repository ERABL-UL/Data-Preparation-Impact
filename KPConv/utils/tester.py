...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=100, debug=False):
        ...
        while True:
            ...
            for i, batch in enumerate(test_loader):
                ...
                in_inds = batch.frame_inds.cpu().numpy()
    ...
    def tree_segmentation_test(self, net, test_loader, config, test_path, num_votes=1, debug=True):
        """
        Test method for slam segmentation models
        """
        ...
        while True:
            ...
            for i, batch in enumerate(test_loader):
                ...
                for b_i, length in enumerate(lengths):
                    ...
                    # Save some prediction in ply format for visual
                    if test_loader.dataset.set == 'validation':
                        print("PUT DATASET ON TEST YOU DUMBASS")
                    else:
                        ...
                        if f_ind % 1 == 0:
                            ...
                            seq_path = join(test_loader.dataset.path, 'plyfiles/sequences', test_loader.dataset.sequences[s_ind])
                            myply = join(seq_path, test_loader.dataset.frames[f_ind] + '.ply')
                            ...
                            predpath = join(test_path, pred_folder, test_loader.dataset.frames[f_ind] + '.ply')
                            write_ply(predpath,[cloud_points, frame_preds,],['x', 'y', 'z', 'preds'])
            ...
            if last_min + 1 < new_min:
                ...
                if test_loader.dataset.set == 'test' and last_min % 1 == 0:
