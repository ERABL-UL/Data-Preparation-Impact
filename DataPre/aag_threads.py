...
if __name__ == "__main__":
    ...
    parser.add_argument('--dia', type=int, default=0.5,
                        help='The initial diameter of the box')
    parser.add_argument('--sample-knn', type=int, default=20000,
    ...
    for file, filename in tqdm(enumerate(points_datapath), total=len(points_datapath)):
        point_cloud_blocks = split_to_blocks(filename, params.block_size, params.overlap, params.knn, filename.split("/")[9], params.sample_knn) # change 9 according to the sequence number in the path
        ...
        while b < length:
            ...
            if length - b >= params.blocks_limit: 
                for pc in point_cloud_blocks[b:b+params.blocks_limit]:
                    t = threading.Thread(target=aag, args=[params.data_dir, pc, params.knn, params.dia, params.sample_knn, filename.split("/")[9], b])
                    t.start()
                    threads.append(t)
                    b += 1
